...
export async function POST(request: Request) {
  try {
    ...
    if (action === "settings") {
      ...
      await db.batch([
        db.prepare("UPDATE raffle_numbers SET status='available',seller_id=NULL,buyer_name=NULL,buyer_phone=NULL,notes=NULL,updated_at=? WHERE seller_id=?").bind(now, id),
      ...
      if (status === "available") {
        await db.prepare("UPDATE raffle_numbers SET status='available',seller_id=NULL,buyer_name=NULL,buyer_phone=NULL,notes=NULL,updated_at=? WHERE number=?").bind(now, number).run();
      } else {
