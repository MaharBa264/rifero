...
export async function POST(request: Request) {
  try {
    ...
    const result = await db.prepare("UPDATE raffle_numbers SET status=?,seller_id=?,buyer_name=?,buyer_phone=?,notes=?,price_cents=?,updated_at=? WHERE number=? AND active=1 AND (status='available' OR seller_id=?)")
      .bind(status, seller.id, buyerName, body.buyerPhone?.trim() ?? "", body.notes?.trim() ?? "", priceCents, now, number, seller.id).run();
    ...
    const used = await db.prepare("SELECT COUNT(*) AS total FROM raffle_numbers WHERE active=1 AND seller_id=? AND status IN ('reserved','sold')").bind(seller.id).first<{ total: number }>();
    return Response.json({ ok: true, numbers: [number], priceCents, status, seller: { ...seller, usedCount: used?.total ?? 0 }, buyerName, buyerPhone: body.buyerPhone?.trim() ?? "", updatedAt: now });
...
export async function DELETE(request: Request) {
  try {
    ...
    const seller = await sellerFromCredentials(body.childName ?? "", body.pin ?? "");
    if (!seller) return apiError(new Error("La sesión venció. Volvé a ingresar."), 401);
    const result = await getD1().prepare("UPDATE raffle_numbers SET status='available',seller_id=NULL,buyer_name=NULL,buyer_phone=NULL,notes=NULL,price_cents=NULL,updated_at=? WHERE number=? AND seller_id=?")
