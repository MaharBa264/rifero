...
export async function POST(request: Request) {
  try {
    ...
    const sales = await getD1().prepare("SELECT number,status,buyer_name,buyer_phone,notes,updated_at FROM raffle_numbers WHERE seller_id=? ORDER BY updated_at DESC")
      .bind(seller.id).all();
