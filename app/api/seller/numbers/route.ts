...
export async function POST(request: Request) {
  try {
    const body = await request.json() as { childName?: string; pin?: string; numbers?: number[]; buyerName?: string; buyerLastName?: string; buyerPhone?: string; buyerEmail?: string; notes?: string; status?: "reserved" | "sold" };
    ...
    await db.prepare("INSERT INTO audit_log (action,actor,payload,created_at) VALUES (?,?,?,?)")
      .bind(`numbers_${status}`, seller.childName, JSON.stringify({ numbers, buyerName, buyerLastName, promo: true }), now).run();
    const used = await db.prepare("SELECT COUNT(*) AS total FROM raffle_numbers WHERE active=1 AND seller_id=? AND status IN ('reserved','sold')").bind(seller.id).first<{ total: number }>();
